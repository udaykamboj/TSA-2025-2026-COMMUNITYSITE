...
export function ApplicationsContent() {
  const { detailedApplications: allDetailedApps, removeDetailedApplication } = useAppStore()
  const { user } = useAuth()
  const targetEmail = user?.email || "demo@example.com"
  const detailedApplications = allDetailedApps.filter(a => a.userEmail === targetEmail)
  ...
  const handleWithdraw = (appId: string) => {
    removeDetailedApplication(appId)
  ...
  const getGoogleMapsUrl = (app: DetailedApplication) => {
...
interface ApplicationCardProps {
  ...
  getGoogleMapsUrl: (app: DetailedApplication) => string
}
