...
function DitheredWaves({
  ...
  const targetFPS = 30; // lower update rate to reduce CPU
  ...
  useFrame(({ clock }) => {
    ...
    // reduce updates when not visible or when animations disabled
    if (!isVisible || disableAnimation) {
      if (disableAnimation) u.time.value = 0;
      return;
    }
  ...
  // throttle pointer updates using requestAnimationFrame
  const pendingMouse = useRef<{ x: number; y: number } | null>(null);
  const handlePointerMove = (e: ThreeEvent<PointerEvent>) => {
    if (!enableMouseInteraction) return;
    const rect = gl.domElement.getBoundingClientRect();
    const dpr = gl.getPixelRatio();
    pendingMouse.current = { x: (e.clientX - rect.left) * dpr, y: (e.clientY - rect.top) * dpr };
    if (rafRef.current == null) {
      rafRef.current = requestAnimationFrame(() => {
        if (pendingMouse.current) {
          mouseRef.current.set(pendingMouse.current.x, pendingMouse.current.y);
